...
namespace PixelSpriteGenerator
{
	public class PsgSprite {
		...
		public PsgSprite(PsgMask mask, PsgOptions options,System.Random rd) {
		...
		private void Init(System.Random rd)
		{
			this.InitContext ();
			this.InitData ();
			this.ApplyMask ();
			this.GenerateRandomSample (rd);
		...
		/// </summary>
		private void GenerateRandomSample(System.Random rd)
		{
			var h = this.height;
			var w = this.width;
			var x = 0;
		...
		/// *
		/// </summary>
		private void RenderPixelData(System.Random rd)
		{
			var isVerticalGradient = rd.Next(0,100)>50;
			var saturation = Mathf.Max(Mathf.Min(rd.Next(0,100)/100.0f * this.options.Saturation, 1f), 0f);

			var hue = rd.Next(0,100)/100.0f;
			...
			for (u = 0; u < ulen; u++)
			{
				...
				isNewColor = Mathf.Abs(((rd.Next(0,100)/100.0f * 2 - 1) 
					+ (rd.Next(0,100)/100.0f * 2 - 1) 
					+ (rd.Next(0,100)/100.0f * 2 - 1)) / 3);
					...
					if (val != 0)
					{
						if (this.options.Colored)
						{
							...
							brightness = Mathf.Sin((u / ulen) * Mathf.PI) * (1f - this.options.BrightnessNoise) 
								+ rd.Next(0,100)/100.0f*3 * this.options.BrightnessNoise; // Original code uses random between 0 and 1.
